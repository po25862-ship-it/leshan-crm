        ...
        <div className="panel" style={{ marginBottom: 24, maxWidth: 620 }}>
          ...
          <form onSubmit={onSubmit}>
            <div style={{ opacity: canEditFull ? 1 : 0.55, pointerEvents: canEditFull ? "auto" : "none" }}>
              ...
              <NeedCriteriaTiers form={form} setForm={setForm} />
            ...
            <div key={tag}>
              ...
              {list.map((item) => {
                ...
                const stats = [
                  ...
                  rangeStatText(item.floorMin, item.floorMax, "樓") && { value: rangeStatText(item.floorMin, item.floorMax, "樓"), label: "樓層" },
                ].filter(Boolean);
                const areaText = (item.areas || []).map(areaLabel).filter(Boolean).join("、");
                return (
                  <div className="card" key={item.id}>
                    <div onClick={() => openEdit(item)} style={{ cursor: "pointer" }}>
